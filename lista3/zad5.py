...
def prostokat(x, dx, dy, a, middlePoint):
    # Translacja
...
def kolko(middlePoint, r):
    for i in range(100):
        for j in range(100):
            if (r ** 2) - r < ((i - middlePoint[0]) ** 2) + ((j - middlePoint[1]) ** 2) < (r ** 2) + r:
                pixelMap[i][j] = 1.0
            if ((i - middlePoint[0]) ** 2) + ((j - middlePoint[1]) ** 2) < (r ** 2) - r:
                pixelMap[i][j] = 0.0
...
def rysujRobota(x):
    ...
    corners.append(int(x[0] + robotRadius))
    corners.append(int(x[1] - robotRadius*4))
    corners.append(int(x[0] - robotRadius))
    corners.append(int(x[1] - robotRadius*4))
    corners.append(int(x[0] - robotRadius))
    corners.append(int(x[1] + robotRadius))
    middlePoint = [(corners[0] + corners[2] + corners[4] + corners[6]) / 4, (corners[1] + corners[3] + corners[5] + corners[7]) / 4]
    prostokat(corners, 0, 0, x[2], middlePoint)
    corners2 = corners
    corners2[3] = corners2[3]+10
    corners2[5] = corners2[5]+10
    prostokat(corners2, 0, 0, x[2]+math.pi/2, middlePoint)
    x = corners
    kolko(middlePoint, 8.5)

# u = uu(tick)
u = [75, 30, 0]
x = np.array([windowSize/2, windowSize/2, 0]).transpose()
y = gg(x, u)
